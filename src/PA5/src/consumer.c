...
static int do_procs(int fd) {
    ...
    size_t elem_size = ioctl(fd, SCULL_IOCGETELEMSZ);
    char* buf; // get the element size, and define the buffer
	...
	for(i = 0; i < g_concurrency; i++) {
		...
		if(pid == 0) {
            ...
            buf = malloc(elem_size + 1); // allocate elem_size + 1 (for null byte)
            if (buf == NULL) { // error check
                perror("malloc");
                exit(EXIT_FAILURE);
            }
            if ((count = read(fd, buf, elem_size)) < 0) {
                perror("read");
            }
            if (count) {
                *(buf + count) = '\0'; // place a null byte immediately after the data
                printf("read: %s\n", buf); // print the data!
